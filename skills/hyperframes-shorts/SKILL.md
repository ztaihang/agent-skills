...
description: 科技财经类 HyperFrames 短视频视觉与版式预处理规范（抖音/小红书竖屏可读）。适用于任意镜头数量的中文解说视频。必须先读本 Skill，再读 hyperframes、hyperframes-cli、gsap；写 HTML 前必须读 css-animations、design-motion-principles、web-typography 与 templates/visual-style-guide.md，每项目须产出 design.md 并轮换视觉风格，禁止套同一模板。中文 TTS 默认 Edge TTS（项目内 generate-tts.py），不需要 hyperframes-media。Agent 须自动跑完全部环境与构建（预检、脚手架、TTS、时间轴、校验），用户只需手动 npm run dev 预览；禁止自动 render 导出 MP4，须等用户确认后再导出。将用户的口播稿按语义拆分为多个场景，生成上屏文案、规划布局、分配时长、指定转场和动效，并统筹配音/BGM/音效编排。
---
...
| 4 | 拆镜 + HTML | `index.html`：按 `design.md` token 写场景、GSAP、字幕、音轨占位 |
...
- 长句（约 >18 字或两句并列）在 `lines.json` **拆成两条**，各生成一条 TTS，避免底部字幕挤成两行
...
- 横屏 1920×1080 字幕字号 **≥ 44px**；竖屏 1080×1920 **≥ 52px**
- 字幕居中全宽，底部留白 40–56px
...
单条口播 >18 字或易挤两行时，在 `lines.json` **拆成两条** TTS（如 s12 + s12b、s17 + s17b），避免底部字幕自动换行。
