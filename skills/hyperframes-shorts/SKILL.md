...
description: 科技财经类 HyperFrames 短视频视觉与版式预处理规范（抖音/小红书竖屏可读）。适用于任意镜头数量的中文解说视频。必须先读本 Skill，再读 hyperframes、hyperframes-cli、gsap；写 HTML 前必须读 design-taste-frontend（taste-skill）、templates/anti-slop-motion-scheme.md、css-animations、design-motion-principles、web-typography 与 templates/visual-style-guide.md，每项目须产出 design.md（含 Motion Plan）并轮换视觉风格，禁止套同一模板。硬性版式：底部字幕永远单行、页面文字不得重叠、禁止 AI 风 emoji/表情包式插图；动效：L0 ambient + L1 入场 + L2 语义对齐 TTS 必做，防廉价静态 PPT。中文 TTS 默认 Edge TTS（项目内 generate-tts.py），不需要 hyperframes-media。Agent 须自动跑完全部环境与构建（预检、脚手架、TTS、时间轴、校验），用户只需手动 npm run dev 预览；禁止自动 render 导出 MP4，须等用户确认后再导出。将用户的口播稿按语义拆分为多个场景，生成上屏文案、规划布局、分配时长、指定转场和动效，并统筹配音/BGM/音效编排。
---
...
9. **`templates/scene-density-guide.md`** — **必读**，镜内防太空、副信息层次
10. **`templates/subtitle-tts-guide.md`** — **必读**，字幕语义拆条 + `speak` 读音
11. **`hyperframes`** + **`gsap`** — 写 HTML composition、GSAP **`mt` 时间轴**动画、字幕（**禁止 ScrollTrigger**）
12. **Agent 自动执行** — TTS、时间轴、SFX 对齐、`npm run check`（见「全自动交付流水线」）
13. **用户手动一步** — `npm run dev` 预览；**仅用户确认后再 render**
...
| 3d | **口播→lines.json** | 读 **`subtitle-tts-guide.md`**：动态字数上限、语义拆条、`speak` 多音字/专有名词；**写满 lines.json 再 TTS** |
| 4 | 拆镜 + HTML | `index.html`：严格按 `design.md` + **scene-density 每镜 5 项**；实现 **L0–L4 动效**；**`.sl` 单行 CSS + 拆条**；GSAP `mt`、字幕、音轨占位 |
...
| 7c | **Post-audit** | 对照 `anti-slop-motion-scheme.md` 门禁 4 + taste pre-flight；补缺失 ambient/入场/转场 |
...
| 层级 | 必做 | 说明 |
|------|------|------|
| **L0** | 每镜 | 背景 ambient 循环（css-animations） |
...
**字幕与 TTS 对齐规则（必须 — 细则见 `templates/subtitle-tts-guide.md`）：**

- 写 `lines.json` **前**读 **`subtitle-tts-guide.md`**；跑 TTS 前脚本会 **校验拆条**，ERROR 则必须改稿
- **`text`（TTS）**：**保留标点**，含每条末尾的 `。！？，` — 逗号/句号控制停顿；**禁止**为迁就字幕删掉 `text` 里的句号
- **`speak`** 可选，仅 TTS；标点规则同 `text`
- **先算动态上限再拆**（读 `meta.json`）：横屏约 ≤16 单位、竖屏约 ≤12；**仅当超长**才拆，20 字、28 字等均按同一规则
- **语义拆条**：允许在句号/逗号/连词/完整子句处断；**禁止**拦腰断（如「让我的」+「电脑…」）
- **上屏 `.sl`**：与 TTS 同句，由 `apply-audio-schedule.mjs` **只去掉该条末尾标点**（`，。！？：；、`）；不是去掉 TTS 里的标点
- **硬性：底部字幕永远单行**——禁止两行、禁止 `<br>` 换行
  - 超出上限必须在 `lines.json` **按语义拆成多条**，各对应一条 `.sl`；**禁止**用 ellipsis 长期挡字
  - 每条 `.sl` CSS **必须**含：`white-space: nowrap; overflow: hidden; text-overflow: ellipsis;`
  - 交付前逐条字幕截帧，确认 **无第二行、无长期 …**
- 多音字（行/重/还/得/了/数/调…）与英文产品名：**必须**检查读音，必要时只改 `speak` 不改 `text`
- 字幕显示时长 = 该条 TTS 时长（`showEnd = start + duration`）
- 字幕之间 **零间隔**：上一条 `opacity:0` 的时刻 = 下一条 `opacity:1` 的时刻
- HTML 音轨用 **单条** `voiceover.wav`；字幕时间轴按 `schedule.json` 逐条 GSAP
...
- [ ] **已读 `scene-density-guide.md`，短镜有副信息 + ambient（非空镜）**？
- [ ] **lines.json 按 `subtitle-tts-guide.md` 拆条；generate-tts 校验 0 ERROR**？
...
### 交付
- [ ] Agent 已自动执行：TTS → apply-audio-schedule → apply-brand（若有片尾）→ **Post-audit** → `verify-index-encoding.py` → GSAP/SFX 对齐 → `npm run check` 通过？
...
交付前在 sc1 起点截帧：**背景必须在缓慢动**；sc1 切入 0.3s 内 **标题须正在入场**。
...
## 长句字幕拆分（语义优先 — 详见 `subtitle-tts-guide.md`）

1. 按 `meta.json` 算 **maxHan**；**≤ 上限 → 一条**（带句末标点给 TTS）
2. **> 上限（常见 20 字以上）** → 必须在语义边界拆，每段仍 ≤ maxHan；**禁止**为凑字数拦腰切
3. **`text` 保留标点给 TTS**；**`.sl` 仅去掉每条末尾标点**（脚本自动）
4. 拆条后 `generate-tts.py` 校验 **0 ERROR**
5. 多音字/专有名词 → `speak` 字段

## 镜内防太空（详见 `scene-density-guide.md`）

- 口播只有 1–2 句的镜 **最常见 AI 丑感**：不是 slop 配色，而是 **画面空**
- 每镜 **至少 5 项**：L0 ambient + 主标题 + 副信息 + L1 stagger + 有意留白
