...
13. **Agent 自动执行** — TTS（含 Edge WordBoundary 对齐）、时间轴、**交付自检脚本**、SFX 对齐、`npm run check`（见「全自动交付流水线」）
...
| 5 | TTS + 字幕对齐 | `python scripts/generate-tts.py` → wav + `schedule.json` + **`alignments.json`（engine=edge-tts）** |
