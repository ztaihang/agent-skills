...
13. **Agent 自动执行** — TTS、时间轴、**交付自检脚本**、SFX 对齐、`npm run check`（见「全自动交付流水线」）
...
```bash
# 项目内推荐：scripts/generate-tts.py（口播逐条 → wav + schedule.json + subtitleParts）
python scripts/generate-tts.py
node scripts/apply-audio-schedule.mjs
```
...
- **无 subtitle 时**：脚本从 `voice` 自动按标点拆多条 `.sl`（如 `s2`、`s2_2`），**共享同一段 wav 时间轴**
...
- 字幕显示时长：多字幕时在同一 wav 的 `[start, showEnd]` 内按权重切分；条间 **零间隔**
...
### 交付
- [ ] Agent 已自动执行：TTS → apply-audio-schedule → apply-brand（若有片尾）→ **`verify-delivery-checklist.py`（0 ERROR）** → **Post-audit** → `verify-index-encoding.py` → GSAP/SFX 对齐 → `npm run check` 通过？
