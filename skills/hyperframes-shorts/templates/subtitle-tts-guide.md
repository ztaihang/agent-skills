...
1. 用 `meta.json` 算 **`maxHan`**（横屏约 **16**，竖屏约 **12**）与 **`minHan`**（横屏 **8**，竖屏 **7**）
2. **`voice` 超长** → **WARN**，TTS 仍整句一条 wav
3. **`subtitle` 每条** → 须 **`minHan ≤ visualUnits ≤ maxHan`**；只在语义边界拆 **显示**
4. 公式：`visualUnits = 汉字数 + ascii×0.55`（标点、空格不计入）
5. **目标区间**：横屏每条 **10–15** 视觉单位为宜；竖屏 **8–11**

| 画幅 | maxHan | minHan | 建议每条 wav 字幕条数 |
|------|--------|--------|----------------------|
| 横屏 1920×1080 | 16 | 8 | **3–6**（超过 **8** 条 → WARN） |
| 竖屏 1080×1920 | 12 | 7 | **3–5**（超过 **7** 条 → WARN） |
...
### 5.3 节奏与密度（解说向 — 防「快闪字幕」）

> **上限防溢出，下限防碎条。** 只满足 `≤ maxHan` 而拆成 8–14 条，观众仍读不过来。

**必须遵守：**

| 规则 | 说明 |
|------|------|
| **下限** | 每条 `visualUnits ≥ minHan`（横屏 8 / 竖屏 7）；**禁止孤词成条** |
| **密度** | 一条 wav 内建议 **3–6** 条字幕；**>8 条（横屏）或 >7 条（竖屏）** 须合并相邻短条 |
| **可读时长** | 每条字幕 ≈ 口播 **1.5–2.5 秒**；同 id 内切换不宜快于每 **1.2 秒** 一条 |
| **英文专名** | `ClaudeCode`、`GitWorktree`、`MCP` 等 **与同句中文同屏**，禁止「ClaudeCode」一条、「在前台当大掌柜」一条 |
| **合并短条** | 相邻两条均 `< minHan` 且口播无长停顿 → **合并为一条**（仍须 ≤ maxHan） |
| **断句优先** | 先按 `。；` 分大段，再按 `，` 拆；**不为凑 maxHan 在子句中间硬切** |

**典型孤词 / 碎条（禁止单独成条）：**

`这一期` · `各司其职` · `在后台` · `分开跑` · `能用的` · `保留` · `因此` · `而且` · `也就是说`
...
**Agent / 用户写 `subtitleParts` 流程：**

1. 按 `voice` 的 `。；` 划 **2–4 个语义块**
2. 块内仅在 `，` 或完整小句处再拆，且每条 ≥ minHan
3. 心算 `visualUnits`；顶满 maxHan 时在 **逗号后** 切，不在词中间切
4. 专名 + 动词尽量同条（如 `由ClaudeCode在前台当大掌柜`）
5. 交付前 `verify-delivery-checklist.py` 会 WARN：**过短条**、**条数过多**、**疑似孤词**
...
- [ ] 每条 `subtitle`/`subtitleParts` 子句 **`minHan ≤ visualUnits ≤ maxHan`**
- [ ] 每条 wav 内字幕条数在建议范围（横屏 3–6，勿 >8）
- [ ] 无孤词碎条（「这一期」「各司其职」等未单独成条）
